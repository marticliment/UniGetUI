...
for lang in downloadedLanguages:
    ...
    for key, value in data.items():
        ...
        if (value != None):
            a += 1
    try:
        langCredits[lang] = data["{0} {0} {0} Contributors, please add your names/usernames separated by comas (for credit purposes)"]
    except KeyError as e:
        print(e)
...
for key in list(langCredits.keys()):
    if langCredits[key] == None:
        langCredits[key] = ""
...
languageCredits = {json.dumps(langCredits, indent=2)}
"""
...
data = ""
for line in f.readlines():
    if (line.startswith("<!-- Autogenerated translators -->")):
        data += line
        for key in list(langCredits.keys()):
            for singleuser in langCredits[key].split(","):
                if singleuser != "" and singleuser.strip() not in data:
                    data += f" - {singleuser.strip()}\n"
        print("  Text modified")
        skip = True
    if (line.startswith("<!-- END Autogenerated translators -->")):
        skip = False
    if (not skip): data += line
if (isSomeChanges):
    f.seek(0)
