...
class WingetUIApplication(QApplication):
    ...
    def __init__(self):
        try:
            ...
            self.LoadingIndicator = IndefiniteProgressBar(self.popup)
            self.LoadingIndicator.setGraphicsEffect(op2)
            self.LoadingIndicator.setStyleSheet(f"""QProgressBar {{border-radius: 2px;height: 4px;border: 0px;background-color: transparent;}}QProgressBar::chunk {{background-color: rgb({'18, 164, 199' if isDark() else '11, 100, 122'});border-radius: 2px;}}""")
            self.LoadingIndicator.setGeometry(QRect(0, 396, 600, 4))
            self.LoadingIndicator.hide()
            self.showAnimation.finished.connect(self.LoadingIndicator.show)

            descriptionEnter.finished.connect(self.LoadingIndicator.show)
    ...
    def loadPreUIComponents(self):
        try:
            ...
            self.callInMain.emit(lambda: self.LoadingIndicator.setValue(1000))
            self.callInMain.emit(lambda: self.loadingText.repaint())
            self.callInMain.emit(lambda: self.LoadingIndicator.repaint())
