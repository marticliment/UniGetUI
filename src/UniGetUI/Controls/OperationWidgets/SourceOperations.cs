    ...
    public class AddSourceOperation : SourceOperation
    {
        ...
        protected override async Task<ProcessStartInfo> BuildProcessInstance(ProcessStartInfo startInfo)
        {
            if (Source.Manager.Capabilities.Sources.MustBeInstalledAsAdmin)
            {
                ...
                startInfo.Arguments = $"\"{Source.Manager.Status.ExecutablePath}\" " + Source.Manager.Properties.ExecutableCallArgs + " " + string.Join(" ", Source.Manager.SourceProvider.GetAddSourceParameters(Source));
            }
            else
            {
                startInfo.FileName = Source.Manager.Status.ExecutablePath;
                startInfo.Arguments = Source.Manager.Properties.ExecutableCallArgs + " " + string.Join(" ", Source.Manager.SourceProvider.GetAddSourceParameters(Source));
            }
        ...
        {
            return Task.Run(() => Source.Manager.SourceProvider.GetAddSourceOperationVeredict(Source, ReturnCode, Output));
        }
    ...
    public class RemoveSourceOperation : SourceOperation
    {
        ...
        protected override async Task<ProcessStartInfo> BuildProcessInstance(ProcessStartInfo startInfo)
        {
            if (Source.Manager.Capabilities.Sources.MustBeInstalledAsAdmin)
            {
                ...
                startInfo.FileName = CoreData.GSudoPath;
                startInfo.Arguments = $"\"{Source.Manager.Status.ExecutablePath}\" " + Source.Manager.Properties.ExecutableCallArgs + " " + string.Join(" ", Source.Manager.SourceProvider.GetRemoveSourceParameters(Source));

            }
            else
            {
                startInfo.FileName = Source.Manager.Status.ExecutablePath;
                startInfo.Arguments = Source.Manager.Properties.ExecutableCallArgs + " " + string.Join(" ", Source.Manager.SourceProvider.GetRemoveSourceParameters(Source));
            }
        ...
        protected override Task<OperationVeredict> GetProcessVeredict(int ReturnCode, string[] Output)
        {
            return Task.Run(() => Source.Manager.SourceProvider.GetRemoveSourceOperationVeredict(Source, ReturnCode, Output));
        }
