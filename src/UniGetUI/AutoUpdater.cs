...
public class AutoUpdater
{
    ...
    {
        if (Settings.Get(Settings.DisableAutoUpdateWingetUI))
        {
    ...
    public static async Task<bool> CheckAndInstallUpdates(Window window, InfoBar banner, bool Verbose, bool AutoLaunch = false, bool ManualCheck = false)
    {
        ...
        try
        {
            ...
            // Check for updates
            string UpdatesEndpoint = Settings.Get(Settings.EnableUniGetUIBeta) ? BETA_ENDPOINT : STABLE_ENDPOINT;
            string InstallerDownloadUrl = Settings.Get(Settings.EnableUniGetUIBeta) ? BETA_INSTALLER_URL : STABLE_INSTALLER_URL;
    ...
    private static async Task<bool> PrepairToLaunchInstaller(string installerLocation, string NewVersion, bool AutoLaunch, bool ManualCheck)
    {
        ...
        if (!ManualCheck && Settings.Get(Settings.DisableAutoUpdateWingetUI))
        {
            Banner.IsOpen = false;
